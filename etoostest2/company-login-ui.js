 ...
 window.etoosCompanyAuth={
   ...
   setLastLogin(data){
     ...
     setTimeout(()=>{
       ...
       if(status){
         status.textContent='임시 비밀번호를 사용 중입니다. 비밀번호 변경 후 계속 사용하는 것을 권장합니다.';
 ...
 if(bar){
   ...
   button.onclick=async()=>{
     ...
     try{
       ...
       alert(error?.name==='AbortError'?'인증 서버 응답이 지연되고 있습니다.':(error.message||String(error)));
     }
   };
   row?.prepend(button);
 }
