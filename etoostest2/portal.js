(()=>{
  ...
  const HOME='./home.html?v=20260810a';
  ...
  async function checkServer(){
    ...
    try{
      ...
      status.textContent=error?.name==='AbortError'?'인증 서버가 8초 안에 응답하지 않습니다. 잠시 후 다시 로그인해 주세요.':`인증 서버 연결 실패 · ${error.message||error}`;
    }
  }

  if(sessionStorage.getItem(TK)){location.replace(HOME);return}
  ...
  form.addEventListener('submit',async event=>{
    ...
    const loginId=document.getElementById('portalLoginId').value.trim().toUpperCase();
    try{
      const response=await fetchWithTimeout(API+'/api/company-auth/login',{
        method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({loginId,password:document.getElementById('portalPassword').value})
      });
      ...
      sessionStorage.setItem(UK,JSON.stringify({
        loginId,
        name:data.profile?.name||data.identity?.name||'',
      ...
      status.className='status success';
      status.textContent=`${data.profile?.name||data.identity?.name||'사용자'}님 확인 완료. MY 247 대문을 여는 중입니다.`;
      location.replace(HOME);
    }catch(error){
      status.className='status error';
      status.textContent=error?.name==='AbortError'?'인증 서버가 12초 안에 응답하지 않았습니다. Worker 배포 상태를 확인해 주세요.':(error.message||String(error));
