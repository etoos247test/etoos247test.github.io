...
export async function loadStudentQuestions() {
  ...
  try {
    const snap = await timeout(
      getDocs(query(collection(db, "questions"), where("studentUid", "==", state.currentUser.uid))),
      10000,
      "질문 조회 시간이 초과되었습니다."
    );
...
export async function submitQuestion(event) {
  ...
  if (!campus || !state.currentProfile?.studentId) {
    showStatus("학생 승인정보가 완성되지 않았습니다. 관리자에게 문의하세요.", "error");
    return;
  }
  ...
  try {
    await timeout(addDoc(collection(db, "questions"), {
      ...
      studentName: state.currentProfile.name ?? state.currentUser.displayName ?? "",
      campus,
      subject,
      questionText,
      status: "waiting",
