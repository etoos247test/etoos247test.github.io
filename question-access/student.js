...
export async function studentLogin(event) {
  ...
  const submit = els.studentLoginForm.querySelector("button"); submit.disabled = true;
  try { await signInWithEmailAndPassword(auth, studentEmail($("studentId").value), $("studentPassword").value); }
  catch (error) {
    const message = error.code === "auth/operation-not-allowed" ? "Firebase에서 이메일/비밀번호 로그인 기능이 아직 켜지지 않았습니다." : error.message ?? String(error);
...
export async function loadStudentQuestions() {
  ...
  try {
    ...
    rows.forEach((data) => {
      ...
      const answer = document.createElement("div"); answer.className = `answer-text ${answered ? "" : "answer-empty"}`; answer.textContent = answered ? data.answer : "아직 답변이 등록되지 않았습니다.";
      item.append(heading, meta, question, answer); els.studentQuestionList.appendChild(item);
    });
  } catch (error) { els.studentQuestionList.innerHTML = `<div class="status error">내 질문을 읽지 못했습니다.\n${error.code ?? ""} ${error.message ?? String(error)}</div>`; }
}
