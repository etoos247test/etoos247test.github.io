...
const DEFAULT_WORK_PERMISSIONS = {
  canManageNotices: true,
  canManageSchedules: true,
  canManageScores: true,
  canManageCounseling: true,
  canAnswerQuestions: true,
  canApproveStudents: false,
  canManageStudentInfo: false
};

function permissionValue(values, key) {
  return typeof values[key] === "boolean" ? values[key] : DEFAULT_WORK_PERMISSIONS[key];
}
...
function permissionFieldset(prefix, values = {}) {
  ...
  CAMPUSES.forEach((campus) => {
    ...
    input.checked = Array.isArray(values.allowedCampuses)
      ? values.allowedCampuses.includes(campus.id)
      : false;
    const text = document.createElement("span");
    text.innerHTML = `${campus.label} 학생 접근<span class="permission-help">이 관의 학생과 업무자료만 노출됩니다.</span>`;
    row.append(input, text);
    box.appendChild(row);
  });

  [
    ["canManageNotices", "공지 입력·수정", "허용된 관의 공지를 등록·수정·삭제합니다."],
    ["canManageSchedules", "시험일정 입력·수정", "허용된 관의 시험일정을 등록·수정·삭제합니다."],
    ["canManageScores", "성적 입력·수정", "허용된 관 학생의 시험 성적을 입력·수정합니다."],
    ["canManageCounseling", "상담 입력·수정", "허용된 관 학생의 상담기록을 입력·수정합니다."],
    ["canAnswerQuestions", "질문 열람·답변", "허용된 관 학생의 질문을 읽고 답변합니다."],
    ["canApproveStudents", "학생 승인·이용중지", "허용된 관에서 학생 승인 상태를 변경하는 준마스터 권한입니다."],
    ["canManageStudentInfo", "학생번호·이름 수정", "허용된 관 학생의 번호와 이름만 수정합니다."]
    ...
    input.checked = permissionValue(values, key);
    const text = document.createElement("span");
...
function readAccess(container) {
  const allowedCampuses = [...container.querySelectorAll("[data-campus]:checked")]
    .map((input) => input.dataset.campus);
  const permissions = {};
...
export async function loadTeacherRequests() {
  ...
  try {
    ...
    snap.forEach((requestDoc) => {
      ...
      const access = permissionFieldset(`pending-${requestDoc.id}`, {
      ...
      approve.textContent = "선택 지점·권한으로 승인";
      ...
      reject.addEventListener("click", () => {
        reviewTeacher(requestDoc.id, data, "rejected", {}, item, approve, reject);
      });
      actions.append(approve, reject);
      item.append(title, meta, access, actions);
      els.teacherRequestList.appendChild(item);
    });
  } catch (error) {
...
async function reviewTeacher(uid, data, decision, access, item, approve, reject) {
  ...
  try {
    ...
    showStatus(
      decision === "approved"
        ? "선택한 지점과 업무 권한으로 교사 승인이 완료되었습니다."
        : "교사 요청을 반려했습니다.",
      decision === "approved" ? "success" : "warning"
    );
    if (!els.teacherRequestList.children.length) {
      els.teacherRequestList.innerHTML = "<div class='status success'>승인 대기 중인 교사 요청이 없습니다.</div>";
    }
  ...
  try {
    ...
    snap.docs
      ...
      .forEach((teacherDoc) => {
        ...
        const item = document.createElement("article");
        item.className = "item teacher-rights-form";
        const title = document.createElement("h3");
        title.textContent = data.name || "이름 정보 없음";
        ...
        save.textContent = "지점·권한 변경 저장";
        const toggle = document.createElement("button");
        toggle.className = data.active === true ? "reject" : "approve";
        toggle.textContent = data.active === true ? "교사 이용 중지" : "교사 이용 재개";
        ...
        toggle.addEventListener("click", () => {
          toggleTeacher(teacherDoc.id, data.name || "교사", data.active !== true, item, toggle);
        });
        actions.append(save, toggle);
        item.append(title, meta, access, actions);
        els.approvedTeacherList.appendChild(item);
      });
  } catch (error) {
...
async function saveTeacherAccess(uid, access, button) {
  ...
  try {
    ...
    showStatus("교사의 관리 지점과 모든 업무 권한을 저장했습니다.", "success");
  } catch (error) {
