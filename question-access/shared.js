...
export function studentCodeRange(campus) {
  return campus === "suseong1"
    ? "M001~M199"
    : campus === "suseong2"
      ? "S001~S199"
      : "M001~M199 또는 S001~S199";
}
...
export function isAnswered(data) {
  return data.status === "answered" && Boolean((data.answer ?? "").trim());
}

export function normalizedStatus(data) {
  return isAnswered(data) ? "answered" : "waiting";
}

export function isMaster() {
  return state.currentProfile?.role === "master" && state.currentProfile?.active === true;
}

export function allowedCampuses() {
  if (isMaster()) return CAMPUSES.map((x) => x.id);
  const values = Array.isArray(state.currentProfile?.allowedCampuses)
    ? state.currentProfile.allowedCampuses
    : [];
  return CAMPUSES.map((x) => x.id).filter((id) => values.includes(id));
}

export function canAccessCampus(campus) {
  return isMaster() || allowedCampuses().includes(campus);
}

function enabledByDefault(field) {
  if (isMaster()) return true;
  return state.currentProfile?.role === "teacher"
    && state.currentProfile?.active === true
    && state.currentProfile?.[field] !== false;
}

export function canManageNotices() { return enabledByDefault("canManageNotices"); }
export function canManageSchedules() { return enabledByDefault("canManageSchedules"); }
export function canManageScores() { return enabledByDefault("canManageScores"); }
export function canManageCounseling() { return enabledByDefault("canManageCounseling"); }
export function canAnswerQuestions() { return enabledByDefault("canAnswerQuestions"); }
...
export function isQuasiMaster() {
  return !isMaster() && (canApproveStudents() || canManageStudentInfo());
}
...
export function questionsForStudent(uid) {
  return uid === "all"
    ...
    : state.teacherQuestions.filter((q) => q.studentUid === uid);
}
