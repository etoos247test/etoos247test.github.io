...
function ensureMenu() {
  ...
  style.textContent = `
    ...
    @media(max-width:600px){.teacher-approval-state{align-items:flex-start;flex-direction:column}.teacher-approval-state span{text-align:left}}
...
function hasApprovalManagementPermission() {
...
function setNativeVisibility(mode) {
  ...
  parts.approval?.classList.toggle("hidden", !approvalMode);
...
function teacherRoleLabel() {
  if (isMaster()) return "마스터";
  if (isQuasiMaster()) return "준마스터 교사";
  return "일반 교사";
}
...
export function showStudentRoleMenu() {
  ...
  menu.innerHTML = `<h2>학생 개인 메뉴</h2><p>본인에게 허용된 자료만 확인할 수 있습니다.</p>
    <div class="role-dashboard-grid">
      ...
      <a class="role-dashboard-card ready" href="../academy-board/?tab=notice"><b>학원공지</b><span>소속관과 1·2관 공통 공지 열람</span></a>
