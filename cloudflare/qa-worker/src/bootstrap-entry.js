...
const BOOTSTRAP_VERSION = 'master-bootstrap-20260810-company-auth';
const COMPANY_AUTH_VERSION = 'company-id-auth-20260810b';
let companyAuthTablesReady = null;

function nowIso() {
  return new Date().toISOString();
}

async function ensureCompanyAuthTables(env) {
  if (!companyAuthTablesReady) {
    companyAuthTablesReady = env.DB.exec(`
    ...
    `).catch((error) => {
      companyAuthTablesReady = null;
      throw error;
    });
  }
  return companyAuthTablesReady;
}
...
async function handleCompanyAuth(request, env) {
  ...
  try {
    await ensureCompanyAuthTables(env);
